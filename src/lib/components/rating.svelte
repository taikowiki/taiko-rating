<script lang="ts" context="module">
	function getCompensated(score: number): number {
		if (score < 600000) {
			return mathjs
				.subtract(mathjs.exp(mathjs.multiply(mathjs.divide(mathjs.log(400001), 600000), score)), 1)
				.valueOf();
			...
			return mathjs
				.add(
					mathjs.multiply(
						...
						mathjs.log(mathjs.add(mathjs.divide(mathjs.subtract(score, 950000), 10000), 1))
					),
					950000
				)
				.valueOf();
	...
	function getRating(
        ...
        let average = Math.round((firstSum + secondSum) / 50);

        let otherSum = 0;
        if(ratings.length > 50){
            for (let i = 50; i < ratings.length; i++) {
	...
	scoreDatas.forEach((song) => {
		...
		Object.keys(song.difficulty).forEach((diff) => {
			...
			ratings.push({
				...
				rating: Math.round(
					mathjs.multiply(measure['상수'], getCompensated(difficulty.score), bonus).valueOf() / 1000
				),
				title: measure['곡명'],
